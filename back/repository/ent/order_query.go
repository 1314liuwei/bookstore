...
func (oq *OrderQuery) QueryBook() *BookQuery {
	query := &BookQuery{config: oq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (oq *OrderQuery) WithBook(opts ...func(*BookQuery)) *OrderQuery {
	query := &BookQuery{config: oq.config}
	for _, opt := range opts {
		opt(query)
	}
